...
with st.spinner('Please wait, application is initializing...'):
    ...
    DB_PATH = '/home/jovyan/DL/topic_09/app/data/db'
        ...
        with st.spinner('Please wait...'):
            ...
            try:
                ...
                if isinstance(results, list):
                    for i, res in enumerate(results):
                        emotion = res['dominant_emotion']
                        st.write(f'Face {i+1}: {emotion}')
                else:
                    emotion = results['dominant_emotion']
                    st.write(f'Face 1: {emotion}')
                
                # Logging the emotions detected
                emotions_detected = [res['dominant_emotion'] for res in results] if isinstance(results, list) else [results['dominant_emotion']]
                msg = '{} - file "{}" emotions detected: {}\n'.format(
                    datetime.datetime.now(),
                    file_name,
                    ', '.join(emotions_detected)
                )
                with open('history.log', 'a') as file:
                    file.write(msg)
                ...
                # Logging the absence of faces
                msg = '{} - file "{}" no faces detected for emotion analysis\n'.format(
                    datetime.datetime.now(),
                    file_name
                )
                with open('history.log', 'a') as file:
